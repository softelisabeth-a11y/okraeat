...
function Plat({ src, nomPlat, mtantPlat, id, plat, donneesRp }) {
  ...
  const platPopulaire = async (id_modif, reconnu) => {
    await axios
      .put("http://localhost:5000/okraeat//repasPopulaire/" + id_modif, {
        populaire: reconnu,
      })
      .then((res) => {
...
const modTable =plat.filter((mod)=>(  mod.id_rep === id  ))
  ...
  return (
    <ContainerPlat>
      <div className="container_bouton">
        ...
        />
        <BoutonModifier className="bouton_update" onClick={()=>{activeUpdate(id)}}>
          <PiPencilSimpleLineFill className="icon_modifier" />
          <span className="texte_modifier">Modifier</span>
        </BoutonModifier>
      </div>
      <div className="detail_plat">
        <span className="nom_plat">{nomPlat}</span>
        <span className="montant_plat">{mtantPlat}</span>
        <p>
          <span className="repas_populaire">Repas populaire</span>
          <Toggle
            defaultChecked
            size="sm"
            onChange={(checked, e) => platPopulaire(id, checked)}
          />
        </p>
      </div>
